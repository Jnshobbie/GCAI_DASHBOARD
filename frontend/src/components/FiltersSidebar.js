...
export default function FiltersSidebar({
  ...
  function handle(field, val) {
    const newValue = { ...safeValue, [field]: val };
    console.log("Filters changed:", newValue); // Debugging line
    onChange(newValue);
  }

  const regionOptions = Array.isArray(locations)
    ? locations.filter((l) => l.scope === "Region")
    : [];
  const countryOptions = Array.isArray(locations)
    ? locations.filter((l) => l.scope === "Country")
    : [];
  ...
  return (
    <aside className="flex flex-col bg-gray-800 rounded-lg p-4 space-y-4 w-full md:w-64 lg:w-72">
      ...
      <div>
        ...
        {safeValue.locationScope === "Region" && (
          <select
            className="w-full bg-gray-700 p-2 rounded"
            value={safeValue.locationName}
        ...
        {safeValue.locationScope === "Country" && (
          <select
            className="w-full bg-gray-700 p-2 rounded"
            value={safeValue.locationName}
            onChange={(e) => handle("locationName", e.target.value)}
